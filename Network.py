...
class Deeplab_v2(object):
    ...
    def build_encoder(self):
        ...
        outputs = self._bottleneck_resblock(outputs, 256, '2a', identity_connection=False)  # [4, 81, 81, 256]
        outputs = self._bottleneck_resblock(outputs, 256, '2b')  # [4, 81,  81, 256]
        outputs = self._bottleneck_resblock(outputs, 256, '2c')  # [4, 81,  81, 256]
        print("after block1:", outputs.shape)
        outputs = self._bottleneck_resblock(outputs, 512, '3a', half_size=True,
                                            identity_connection=False)  # [4, 41,  41, 512]
        for i in range(1, 4):
            outputs = self._bottleneck_resblock(outputs, 512, '3b%d' % i)  # [4, 41,  41, 512]
        print("after block2:", outputs.shape)
        outputs = self._dilated_bottle_resblock(outputs, 1024, 2, '4a', identity_connection=False)  # [4, 41, 41, 1024]
        for i in range(1, 23):
            outputs = self._dilated_bottle_resblock(outputs, 1024, 2, '4b%d' % i)  # [4, 41, 41, 1024]
        print("after block3:", outputs.shape)
        outputs = self._dilated_bottle_resblock(outputs, 2048, 4, '5a', identity_connection=False)  # [4, 41, 41, 2048]
    ...
    def build_decoder(self, encoding):
        print("-----------build decoder-----------")
        outputs = self._ASPP(encoding, self.num_classes, [6, 12, 18, 24])  # [4, 41, 41, 21]
    ...
    def _bottleneck_resblock(self, x, num_o, name, half_size=False, identity_connection=True):
        ...
        if not identity_connection:
            ...
            o_b1 = x
        # branch2
        o_b2a = self._conv2d(x, 1, num_o / 4, first_s, name='res%s_branch2a' % name)  # [4, 81, 81, 64]
        o_b2a = self._batch_norm(o_b2a, name='bn%s_branch2a' % name, is_training=False, activation_fn=tf.nn.relu)

        o_b2b = self._conv2d(o_b2a, 3, num_o / 4, 1, name='res%s_branch2b' % name)  # [4, 81, 81, 64]
        o_b2b = self._batch_norm(o_b2b, name='bn%s_branch2b' % name, is_training=False, activation_fn=tf.nn.relu)

        o_b2c = self._conv2d(o_b2b, 1, num_o, 1, name='res%s_branch2c' % name)  # [4, 81, 81, 256]
    ...
    def _dilated_bottle_resblock(self, x, num_o, dilation_factor, name, identity_connection=True):
        ...
        if not identity_connection:
            ...
            o_b1 = x
        # branch2
        o_b2a = self._conv2d(x, 1, num_o / 4, 1, name='res%s_branch2a' % name)  # [4, 41, 41, 256]
        o_b2a = self._batch_norm(o_b2a, name='bn%s_branch2a' % name, is_training=False, activation_fn=tf.nn.relu)

        o_b2b = self._dilated_conv2d(o_b2a, 3, num_o / 4, dilation_factor,
                                     name='res%s_branch2b' % name)  # [4, 41, 41, 256]
        o_b2b = self._batch_norm(o_b2b, name='bn%s_branch2b' % name, is_training=False, activation_fn=tf.nn.relu)

        o_b2c = self._conv2d(o_b2b, 1, num_o, 1, name='res%s_branch2c' % name)  # [4, 41, 41, 1024]
        o_b2c = self._batch_norm(o_b2c, name='bn%s_branch2c' % name, is_training=False, activation_fn=None)
        # add
        outputs = self._add([o_b1, o_b2c], name='res%s' % name)
        # relu
    ...
    def _conv2d(self, x, kernel_size, num_o, stride, name, biased=False):
        ...
        with tf.variable_scope(name):
            ...
            o = tf.nn.conv2d(x, w, [1, stride, stride, 1], padding='SAME')
            if biased:
                o = tf.nn.bias_add(o, tf.get_variable('biases', shape=[num_o]))
            return o
    ...
    def _max_pool2d(self, x, kernel_size, stride, name):
        return tf.nn.max_pool(x, [1, kernel_size, kernel_size, 1], [1, stride, stride, 1], padding='SAME', name=name)

    def _batch_norm(self, x, name, is_training, activation_fn, trainable=False):
        # 对于小批次，最好设置 BN layers (running means and variances) frozen,
        # 设置 is_training=False 后训练不修改 pre-trained model 提供的（均值和方差的值）
        # 注意设置为 is_training=False 仍然会更新 gamma (scale) and beta (offset)
        # 如果想要这两个参数不更新 ,设置trainable = False
        with tf.variable_scope(name) as scope:
            return tf.contrib.layers.batch_norm(x, scale=True, activation_fn=activation_fn,
