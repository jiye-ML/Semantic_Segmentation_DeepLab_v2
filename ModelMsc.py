...
class Model_msc(object):
    ...
    def train(self):
        ...
        for step in range(self.conf.num_steps + 1):
            ...
            if step % self.conf.save_interval == 0:
                images, labels, summary, _ = self.sess.run([self.image_batch, self.label_batch,
                                                            self.total_summary, self.train_op], feed_dict=feed_dict)
    ...
    def test(self):
        ...
        for step in range(self.conf.valid_num_steps):
            preds, _, _, c_matrix = self.sess.run(
    ...
    def train_setup(self):
        ...
        if self.conf.encoder_name not in ['res101', 'res50', 'deeplab']:
            ...
            sys.exit(-1)
        elif self.conf.encoder_name == 'deeplab':  # 多尺度融合
            ...
            restore_var = [v for v in tf.global_variables() if 'fc' not in v.name]  # fc 是最后ASPP所有层的变量前缀
            # Trainable Variables
        ...
        raw_output = tf.reduce_max(tf.stack([raw_output100,
                                             ...
                                             tf.image.resize_images(raw_output05, tf.shape(raw_output100)[1:3, ])]),
                                   axis=0)  # 每个预测是多尺度图片输出结果的最大值

        # Groud Truth: ignoring all labels greater or equal than n_classes
        label_proc = prepare_label(self.label_batch, tf.stack(raw_output.get_shape()[1:3]),
                                   num_classes=self.conf.num_classes, one_hot=False)  # [batch_size, h, w]
        label_proc075 = prepare_label(self.label_batch, tf.stack(raw_output075.get_shape()[1:3]),
                                      num_classes=self.conf.num_classes, one_hot=False)
        label_proc05 = prepare_label(self.label_batch, tf.stack(raw_output05.get_shape()[1:3]),
                                     num_classes=self.conf.num_classes, one_hot=False)
        ...
        # L2 regularization 权值衰减
        l2_losses = [self.conf.weight_decay * tf.nn.l2_loss(v) for v in all_trainable if 'weights' in v.name]
        # Loss function
        self.reduced_loss = tf.reduce_mean(loss) + tf.reduce_mean(loss100) + tf.reduce_mean(loss075) + tf.reduce_mean(
            loss05) + tf.add_n(l2_losses)

        # Define optimizers
        # 'poly' learning rate 学习率衰减
        ...
        accum_grads = [tf.Variable(tf.zeros_like(v.initialized_value()), trainable=False) for v in
        ...
        self.pred = tf.expand_dims(tf.argmax(tf.image.resize_bilinear(raw_output, input_size), axis=3), dim=3)
        # Image summary.
    ...
    def test_setup(self):
        ...
        image_batch_075 = tf.image.resize_images(self.image_batch, tf.stack(
            [tf.to_int32(tf.multiply(h_orig, 0.75)), tf.to_int32(tf.multiply(w_orig, 0.75))]))
        image_batch_05 = tf.image.resize_images(self.image_batch, tf.stack(
        ...
        self.pred = tf.reshape(tf.expand_dims(raw_output, dim=3), [-1, ])
        # labels
        ...
        # Pixel accuracy
        self.accu, self.accu_update_op = tf.contrib.metrics.streaming_accuracy(self.pred, gt, weights=weights)
    ...
    def predict_setup(self):
        ...
        image_batch_075 = tf.image.resize_images(image_batch, tf.stack(
            [tf.to_int32(tf.multiply(h_orig, 0.75)), tf.to_int32(tf.multiply(w_orig, 0.75))]))
        image_batch_05 = tf.image.resize_images(image_batch, tf.stack(
            [tf.to_int32(tf.multiply(h_orig, 0.5)), tf.to_int32(tf.multiply(w_orig, 0.5))]))
